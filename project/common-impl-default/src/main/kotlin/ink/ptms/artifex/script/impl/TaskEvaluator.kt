...
import ink.ptms.artifex.script.event.ScriptEvaluateEvent
 ...
 */
class TaskEvaluator(
    private val meta: ScriptMeta,
    private val sender: ProxyCommandSender,
    private val loggingRunning: Boolean,
    ...
    override fun apply(property: ScriptRuntimeProperty, report: Consumer<ScriptResult.Diagnostic>?): Boolean {
        ...
        result.reports().forEach { r ->
            if (report != null) {
                ...
                Artifex.api().getScriptHelper().printScriptResult(r, sender, detailError)
            }
        }
        // 汇报运行结果
        if (loggingRunning) {
